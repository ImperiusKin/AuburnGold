# Launches tileset_fixer_gui.py inside WSL, with its window shown on the
# Windows desktop via WSLg. Meant to be started by TilesetFixerGUI.bat
...
# \\wsl.localhost\Ubuntu\home\belial\AuburnGold\dev_scripts\tileset_fixer
...
$scriptPath = "$linuxPath/tileset_fixer_gui.py"
...
if ($exitCode -ne 0) {
    Write-Host ""
    Write-Host "tileset_fixer_gui.py exited with code $exitCode."
