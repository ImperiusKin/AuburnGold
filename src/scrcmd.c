...
bool8 ScrCmd_checkfieldmove(struct ScriptContext *ctx)
{
    ...
    for (u32 i = 0; i < PARTY_SIZE; i++)
    {
        ...
        bool8 isEgg = GetMonData(&gParties[B_TRAINER_PLAYER][i], MON_DATA_IS_EGG);
        if (!species || isEgg)
            break;
        if (CanLearnTeachableMove(species, move) || MonKnowsMove(&gParties[B_TRAINER_PLAYER][i], move) == TRUE)
        {
