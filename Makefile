...
AUTO_GEN_TARGETS += $(patsubst %.pory,%.inc,$(shell find $(DATA_ASM_SUBDIR)/ -type f -name '*.pory'))
...
%.smol:     %      ; $(SMOL) -w $< $@
%.rl:       %      ; $(GFX) $< $@
$(DATA_ASM_SUBDIR)/%.inc: $(DATA_ASM_SUBDIR)/%.pory ; $(SCRIPT) -i $< -o $@ -fc $(TOOLS_DIR)/poryscript/font_config.json -cc $(TOOLS_DIR)/poryscript/command_config.json
